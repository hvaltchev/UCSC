...
class Clock:
    ...
    def __str__(self):
        return f"{self.hours:2d}:{self.minutes:02d}"
...
def main():
    ...
    for hours in range(-2, 25, 2):
        for minutes in range(-10, 10):
            ...
            # both are tested
            cmp_value = int(c2)
            assert Clock(int(c2)) == c2, f"int not right: {c2}"
            c_sum = c1 + c2
            c_diff = c1 - c2
            c3 = c_sum + c_diff # should be 2 * c1
            c4 = Clock(2* c1.hours, 2 * c1.minutes)
            assert c3 == c4, f"+/-/== error: {c1} +/- {c2}"
            c5 = -c2
            assert c_diff == c1 + c5
    hours, minutes = 2, 30
    clocks = [Clock(hours, minutes)]
    clocks += [Clock((hours, minutes))]
    clocks += [Clock(f"{hours}:{minutes:2d}")]
