#!/usr/bin/env python3
"""lab19_1.py A SortedDictionary class with only "description"
...
class SortedDictionary(dict): 
    ...
    def __setattr__(self, attribute_name, value):
        if attribute_name not in SortedDictionary.allowed_attributes:
            raise TypeError(f"Can't set attributes of class {self.__class__.__name__}")
...
def main():
    ...
    for initializer in (
        ...
        print(f" regular_dict.keys(): {list(regular_dict.keys())}")
        print(f"  sorted_dict.keys(): {list(sorted_dict.keys())}")
        print(f"""sorted_dict for-loop: {", ".join([
            str(k) for k in sorted_dict])}""")

    sorted_dict.description = "Fourth Day of Christmas"
    print(f"sorted_dict.description = {sorted_dict.description}")
    try:
